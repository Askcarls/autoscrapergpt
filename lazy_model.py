...
from nemo_wrapper import Nemollm
...
class LazyModel:
    ...
    def model(self):
        if self._model is None:
            ...
            elif self.config['type'] == 'Nemollm':
                #self._model = Nemollm(model_id=self.config['model_id'], temperature=self.config['temperature'])
                model_kwargs = self.config.copy()
                model_kwargs.pop('type', None)
                model_kwargs.pop('name', None)
                self._model = Nemollm(**model_kwargs)
        return self._model
